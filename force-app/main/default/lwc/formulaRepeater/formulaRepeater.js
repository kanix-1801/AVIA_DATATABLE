...
@track summaryRows = [
  ...
  handleSummaryChange(event) {
    ...
    // Default Decimal_Value to 0 if blank or not a valid number
    if (fieldName === 'Decimal_Value') {
        const parsedValue = parseInt(value, 10);
        this.summaryRows[index][fieldName] = isNaN(parsedValue) ? 0 : parsedValue;
    } else {
        this.summaryRows[index][fieldName] = value;
    }
...
addSummaryRow() {
    ...
    this.summaryRows = [...this.summaryRows, { id, SField: '', operation: '', Summary_Column_Name: '',Decimal_Value: 0 }];
}
...
@api
  get summaryFieldValues() {
    ...
    this.summaryRows.forEach((row) => {
        ...
        const { Summary_Column_Name, SField, operation,Decimal_Value} = row;
      ...
      if (SField && operation) {
        ...
        //summaryMap[Summary_Column_Name] = `{${temp},${operation},${Decimal_Value ?? 0}}`;
        summaryMap[Summary_Column_Name] = `{${temp},${operation},${(Decimal_Value ?? 0) >= 10 ? 9 : (Decimal_Value ?? 0)}}`;
